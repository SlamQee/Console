...
UARTStdioConfig(uint32_t ui32PortNum, uint32_t ui32Baud, uint32_t ui32SrcClock)
{
    ...
    //
    // Configure the UART for operation;
    //
...
UARTStdioIntHandler(void)
{
    ...
    if(ui32Ints & (UART_INT_RX | UART_INT_RT))
    {
        ...
        while(MAP_UARTCharsAvail(g_ui32Base))
        {
            ...
            //ROM_UARTCharPutNonBlocking(UART0_BASE, cChar);
#ifdef DEBUG_CMD_ANALYZE
			//UartConnection(cChar);
			UartDebugRxProc(cChar);	
#endif
            //
