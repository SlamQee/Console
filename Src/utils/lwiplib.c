...
lwIPLinkDetect(void)
{
    ...
    if(bHaveLink)
    {
        ...
        //
#if LWIP_DHCP
        if(g_ui32IPMode == IPADDR_USE_DHCP && Eeprom_Active_Value.IpAddrTypeAuto)
        {
            dhcp_start(&g_sNetIF);
        }
#endif
        ...
        //
#if LWIP_AUTOIP
        if(g_ui32IPMode == IPADDR_USE_AUTOIP && Eeprom_Active_Value.IpAddrTypeAuto)
        {
...
lwIPServiceTimers(void)
{
    ...
    //
#if LWIP_AUTOIP
    if((g_ui32LocalTimer - g_ui32AutoIPTimer) >= AUTOIP_TMR_INTERVAL && Eeprom_Active_Value.IpAddrTypeAuto)
    {
    ...
    //
#if LWIP_DHCP
    if((g_ui32LocalTimer - g_ui32DHCPCoarseTimer) >= DHCP_COARSE_TIMER_MSECS && Eeprom_Active_Value.IpAddrTypeAuto)
    {
    ...
    //
#if LWIP_DHCP
    if((g_ui32LocalTimer - g_ui32DHCPFineTimer) >= DHCP_FINE_TIMER_MSECS && Eeprom_Active_Value.IpAddrTypeAuto)
    {
    ...
    //
#if LWIP_AUTOIP || LWIP_DHCP
    if((g_ui32LocalTimer - g_ui32LinkTimer) >= LINK_TMR_INTERVAL && Eeprom_Active_Value.IpAddrTypeAuto)
    {
...
lwIPPrivateInit(void *pvArg)
{
    ...
    //
#if !NO_SYS && (LWIP_AUTOIP || LWIP_DHCP)
	if(Eeprom_Active_Value.IpAddrTypeAuto)
	{
		sys_timeout(LINK_TMR_INTERVAL, lwIPPrivateLinkTimer, NULL);
	}
#endif
...
lwIPPrivateNetworkConfigChange(void *pvArg)
{
...
#if LWIP_DHCP || LWIP_AUTOIP
    else if(Eeprom_Active_Value.IpAddrTypeAuto)
    {
        ip_addr.addr = 0;
        net_mask.addr = 0;
        gw_addr.addr = 0;
    ...
    switch(g_ui32IPMode)
    {
        ...
        case IPADDR_USE_STATIC:
        {
            ...
            //
#if LWIP_DHCP
            if((ui32IPMode == IPADDR_USE_DHCP) && g_bLinkActive && Eeprom_Active_Value.IpAddrTypeAuto)
            {
                dhcp_start(&g_sNetIF);
            }
#endif
            ...
            //
#if LWIP_AUTOIP
            if((ui32IPMode == IPADDR_USE_AUTOIP) && g_bLinkActive && Eeprom_Active_Value.IpAddrTypeAuto)
            {
                autoip_start(&g_sNetIF);
            }
#endif
        ...
        {
			if(!Eeprom_Active_Value.IpAddrTypeAuto)	break;
            //
